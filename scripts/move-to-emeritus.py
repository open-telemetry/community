...
def _has_active_member(document, user_info):
    ...
    for role, section_name in ROLE_SECTIONS.items():
        targets = {
            user.lower() for user, info in user_info.items()
...
def _find_membership_document(repo, user_info):
    ...
    # Keep the follow-up issue path after an active entry moves to Emeritus.
...
def create_emeritus_prs(inactive_report, repo_warnings, cutoff):
    ...
    for repo, inactive_list in sorted(inactive_report.items()):
        ...
        if result is None:
            print(f"  Could not fetch README or CONTRIBUTING for {ORG}/{repo}, skipping.")
            continue
