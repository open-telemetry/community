...
resource "github_team" "triagers" {
  count       = length(var.triagers) > 0 ? 0 : 1
...
resource "github_team" "approvers" {
  ...
  parent_team_id = length(var.triagers) > 0 ? github_team.triagers[0] : null
}
...
resource "github_team" "maintainers" {
  ...
  privacy     = var.privacy  // or "secret"
  parent_team_id = length(var.approvers) > 0 ? github_team.approvers[0] : null
}
